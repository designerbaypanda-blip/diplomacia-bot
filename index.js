...
// Proxy endpoint to bypass DNS issues on the phone
app.get('/proxy/*', async (req, res) => {
  try {
    const targetUrl = 'https://green-math-7b4d.designerbaypanda.workers.dev/' + req.params[0];
...
async function performSync() {
  ...
  try {
    const marketResp = await axios.get(`https://green-math-7b4d.designerbaypanda.workers.dev/api/market?page=1&t=${timestamp}`);
    if (marketResp.data && marketResp.data.listings) {
      for (const item of marketResp.data.listings) {
        const resName = item.resource;
        const price = parseFloat(item.unit_price) || 0;
        if (price > 0 && cheapestPrices[resName] !== undefined) {
          if (cheapestPrices[resName] === 0 || price < cheapestPrices[resName]) {
            cheapestPrices[resName] = price;
          }
        }
      }
    }
  ...
  try {
    const leaderResp = await axios.get(`https://green-math-7b4d.designerbaypanda.workers.dev/api/countries/leaderboard/countries?t=${timestamp}`);
    if (leaderResp.data && Array.isArray(leaderResp.data.countries)) {

      for (const country of leaderResp.data.countries) {
        let countryWealth = parseFloat(country.treasury) || 0;
