...
async function performSync() {
  ...
  try {
    const marketResp = await axios.get(`https://diplomacia.com.tr/api/market?limit=100&t=${timestamp}`);
    if (marketResp.data && marketResp.data.items) {
    }
  } catch(e) {}
  ...
  for (const res of resources) {
    try {
      const resp = await axios.get(`https://diplomacia.com.tr/api/market/resource/${res}?t=${timestamp}`);
  ...
  try {
    const leaderResp = await axios.get(`https://diplomacia.com.tr/api/leaderboard?limit=300&t=${timestamp}`);
...
app.listen(port, () => {
  ...
  // İlk açılışta hemen 1 kere çalıştır
  performSync().catch(e => console.error("Initial sync error:", e));
  
  // Sonra her 1 dakikada (60000 ms) bir otomatik olarak sync işlemini tetikle
  setInterval(async () => {
    try {
      await performSync();
    } catch (e) {
